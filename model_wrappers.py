...
class doublefid:  # the original 2-step architecture
    ...
    # LF output in PCA space (PCA coefficients)
...
class goku_df_ratio:
    def __init__(self, path_LF, path_LHr, device='cpu', bounds_path="./data/input_limits-W.txt"):
        self.device = torch.device(device)
...
class boost:  # single fid model for boost 
    def __init__(self, path, device='cpu', bounds_path="./data/input_limits-W.txt"):
        self.device = torch.device(device)
    ...
    def predict(self, x):
        ...
        x = (x - self.bounds[:,0]) / (self.bounds[:,1] - self.bounds[:,0])
        # lg to linear
        lgk, y = self.single.predict(x)
        # reshape y_LF(n_sample, n_k * n_z) back to (n_sample, n_z, n_k) if needed: len(lgk) = n_k < y_LF.shape[1]
        if len(lgk) < y.shape[1]:
            y = y.reshape(y.shape[0], -1, len(lgk))
        return 10**lgk, y
...
# class gokunet(mfbox):
#     def __init__(self, path_L1, path_L2, path_LH, path_LHf=None, device='cpu', bounds_path="./data/input_limits-W.txt", stitch="XL1L2"):
...
class goku_df(doublefid):  # the original 2-step architecture
    def __init__(self, path_LF, path_LH, device='cpu', bounds_path="./data/input_limits-W.txt"):
...
class goku_df_pca_in(doublefid): # the original 2-step architecture with PCA input (step 2)
    def __init__(self, path_LF, path_LH, device='cpu', bounds_path="./data/input_limits-W.txt"):
        super().__init__(path_LF, path_LH, device=device)
        self.bounds = np.loadtxt(bounds_path)
...
# class gokunet_split():
#     def __init__(self, path_LA, path_HA, path_LB, path_HB, device='cpu', bounds_path="./data/input_limits-W.txt"):
...
# class gokunet_sf
class goku_sf(singlefid):  # can use this instead of "boost"
    def __init__(self, path, device='cpu', bounds_path="./data/input_limits-W.txt", output_orig_scale=False):
    ...
    def predict(self, x):
        ...
        if self.output_orig_scale:
            print('output in original scale')
            return lgk, y
        else:
            return 10**lgk, 10**y
...
# class gokunet_alpha():
#     def __init__(self, path_LA, path_HA, path_L2, path_LH, device='cpu', bounds_path="./data/input_limits-W.txt"):
#         self.part_A = gokunet_df(path_LA, path_HA, device=device, bounds_path=bounds_path)
...
#     def __init__(self, path_LA, path_HA, path_L2, path_HB, device='cpu', bounds_path="./data/input_limits-W.txt"):
#         self.model_LA = gokunet_sf(path_LA, device=device, bounds_path=bounds_path)
#         self.model_HA = gokunet_sf(path_HA, device=device, bounds_path=bounds_path, output_orig_scale=True)
#         self.model_L2 = gokunet_sf(path_L2, device=device, bounds_path=bounds_path)
...
#     def __init__(self, path_LA, path_HAlin, path_HAnonl, path_L2, path_LHlin, path_LHnonl, device='cpu', bounds_path="./data/input_limits-W.txt"):
...
# class gokunet_gamma():
#     def __init__(self, path_LA, path_HA, path_L2, path_L2H, device='cpu', bounds_path="./data/input_limits-W.txt"):
#         self.part_A = gokunet_df(path_LA, path_HA, device=device, bounds_path=bounds_path)
...
#     def __init__(self, path_LA, path_HA, path_L, path_LH, device='cpu', bounds_path="./data/input_limits-W.txt"):
...
#     def __init__(self, path_L, path_HA, path_LH, device='cpu', bounds_path="./data/input_limits-W.txt"):
...
class _Alias:
    def __init__(self, new_cls, old_name):
        self._new_cls = new_cls
        self._old_name = old_name
    def __call__(self, *args, **kwargs):
        warnings.warn(f"'{self._old_name}' is deprecated, use '{self._new_cls.__name__}' instead.",
                      DeprecationWarning, stacklevel=2)
        return self._new_cls(*args, **kwargs)
act_dict = {'ReLU': nn.ReLU(), 'SiLU': nn.SiLU(), 'Tanh': nn.Tanh(), 'None': None}

gokunet_df_ratio = _Alias(goku_df_ratio, 'gokunet_df_ratio')
gokunet_df = _Alias(goku_df, 'gokunet_df')
gokunet_df_pca_in = _Alias(goku_df_pca_in, 'gokunet_df_pca_in')
gokunet_sf = _Alias(goku_sf, 'gokunet_sf')